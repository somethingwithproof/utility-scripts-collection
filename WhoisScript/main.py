...
class DomainFileReader(DomainReader):
    ...
    def read_domains(self) -> List[str]:
        ...
        try:
            ...
            raise DomainReadError(
                f"Input file not found: {self._filepath}"
            ) from None
        except IOError as e:
            logger.error(f"Error reading file {self._filepath}: {e}")
            raise DomainReadError(
                f"Error reading file {self._filepath}: {e}"
            ) from e
        except Exception as e:
            logger.exception(
                f"An unexpected error occurred reading {self._filepath}"
            )
...
class WhoisClient(WhoisLookup):
    ...
    def __init__(
        self,
        server_config: WhoisServerConfig,
        timeout: float = DEFAULT_SOCKET_TIMEOUT,
    ):
        if not server_config:
            raise ValueError(
                "WhoisServerConfig cannot be None for WhoisClient."
            )
    ...
    def lookup(self, domain: str) -> WhoisResult:
        ...
        logger.info(
            f"Performing WHOIS lookup for '{domain}' via {self._server_config.host}"
        )
        query = f"{domain}{LINE_ENDING.decode()}".encode(
            "idna"
        )  # Use IDNA encoding for international domains
        ...
        with closing(socket.socket(socket.AF_INET, socket.SOCK_STREAM)) as s:
            try:
                ...
                logger.info(
                    f"Successfully received WHOIS response for '{domain}'."
                )
                return WhoisResult(
                    domain=domain, success=True, response=response_text.strip()
                )
                ...
                return WhoisResult(
                    domain=domain, success=False, error_message=error_msg
                )
            except (
                socket.gaierror
            ) as e:  # Get address info error (DNS lookup failure)
                ...
                socket.error
            ) as e:  # Other socket errors (connection refused, network unreachable etc)
                ...
                error_msg = (
                    f"Unexpected error during WHOIS lookup for {domain}: {e}"
                )
                logger.exception(
                    error_msg
...
class WhoisApp:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            print(
                f"Error: Could not read domains - {e}", file=sys.stderr
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Perform WHOIS lookups for domains listed in a file."
    )
    parser.add_argument(
        "-f",
        "--file",
    ...
    try:
        ...
        whois_client = WhoisClient(
            server_config=server_config, timeout=args.timeout
        )
        ...
        logger.exception(
            "An unexpected error occurred during application setup."
        )
