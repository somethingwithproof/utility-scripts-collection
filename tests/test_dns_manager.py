...
class TestDNSRecord:
    ...
    def test_dns_record_creation(self):
        ...
        record = DNSRecord(
            name="example.com", record_type="A", value="192.168.1.1", ttl=300
        )
    ...
    def test_dns_record_str(self):
        ...
        record_no_ttl = DNSRecord(
            name="example.com", record_type="A", value="192.168.1.1"
        )
...
"""
        ...
        assert any(
            r.name == "example.com" and r.record_type == "A" for r in records
        )
        assert any(
            r.name == "example.com" and r.record_type == "MX" for r in records
        )
        assert any(
            r.name == "example.com" and r.record_type == "TXT" for r in records
        )
