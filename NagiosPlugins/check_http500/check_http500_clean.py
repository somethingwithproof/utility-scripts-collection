...
class EmailNotifier:
    ...
    def send_alert(self, to_email: str, host: str, error_content: str) -> bool:
...
Examples:
  ...
  %(prog)s --host example.com --port 443 --email alerts@example.com --timeout 60
    ...
    parser.add_argument(
        "--host", required=True, help="Hostname or IP address to check"
    )
    ...
    parser.add_argument(
        "--email", required=True, help="Email address for notifications"
    )
