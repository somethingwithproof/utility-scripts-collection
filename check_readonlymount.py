    ...
    def execute(self, command: str) -> str:
        ...
        try:
            ...
            raise SSHExecutionError(
                f"'ssh' command not found. Is it installed and in PATH?"
            )
        except Exception as e:
            # Catch other potential errors during subprocess execution
            raise SSHExecutionError(
                f"An unexpected error occurred during SSH execution: {e}"
            ) from e
    ...
    def get_remote_mounts(self, mount_tab_path: str) -> List[MountInfo]:
        ...
        for line in raw_output.strip().splitlines():
            ...
            if mount_info:
                ...
                print(
                    f"WARNING: Could not parse mount line: {line}",
                    file=sys.stderr,
                )
        return mounts
    ...
    def filter_mounts(
        ...
        for mount in mounts:
            ...
            if (
                ...
                and mount.filesystem_type in config.exclude_types
            ):
                continue
...
def parse_arguments() -> CheckConfig:
    ...
    parser.add_argument(
        "-sh", required=True, dest="sHost", help="SSH Remote host"
    )
    parser.add_argument(
        "-su",
        dest="sUser",
        default="zenoss",
        help="SSH Remote user (default: zenoss)",
    )
    parser.add_argument(
        "-sp",
        dest="sPort",
    ...
    parser.add_argument(
        ...
        dest="mtabPath",
        default="/proc/mounts",
        help="Use this mtab instead (default is /proc/mounts)",
    )
    parser.add_argument(
        "-partition",
        action="append",
        dest="partFilter",
        help="Glob pattern of path or partition to check (may be repeated)",
    )
    parser.add_argument(
        "-x",
        dest="exclude",
        help="Glob pattern of path or partition to ignore (only works if -partition unspecified)",
    )
    parser.add_argument(
        "-X",
        action="append",
