...
def device_details(request, device_id):
    try:
        ...
        c = Context(
            {
                ...
                "device_templates": dev_templates,
                "device_id": dev.id,
            }
        )
        return HttpResponse(t.render(c))
        ...
        return HttpResponse(
            "No such device! It means that something went wrong."
        )
...
def device_template_details(request, device_id, template_id):
    ...
    for dp in datapoints:
        rrdpf = zenoss_rrd_performance_point.objects.filter(
            device=dev, template=tpl, datasource_details=dp
        )
