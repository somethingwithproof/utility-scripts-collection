...
class SmtpConfig:
    ...
    password: Optional[str] = (
        None  # Consider more secure ways in production (secrets manager)
    )
    ...
    def notify(self, subject: str, body: str) -> None:
        ...
        try:
            ...
            raise NotificationError(
                f"Network error during SMTP connection: {e}"
            ) from e
        except Exception as e:
...
def load_smtp_config_from_env() -> SmtpConfig:
    ...
    password = os.getenv(
        "SMTP_PASSWORD"
    )  # In real-world use secrets management
    ...
    return SmtpConfig(
        ...
        in ("true", "1", "yes"),  # Default to True for security
